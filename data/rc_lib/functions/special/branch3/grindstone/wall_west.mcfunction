scoreboard players set x_min rc_lib.var 36
scoreboard players set x_max rc_lib.var 64
scoreboard players set y_min rc_lib.var 24
scoreboard players set y_max rc_lib.var 40
scoreboard players set z_min rc_lib.var 8
scoreboard players set z_max rc_lib.var 16
function rc_lib:special/3
scoreboard players set x_min rc_lib.var 36
scoreboard players set x_max rc_lib.var 64
scoreboard players set y_min rc_lib.var 24
scoreboard players set y_max rc_lib.var 40
scoreboard players set z_min rc_lib.var 48
scoreboard players set z_max rc_lib.var 56
function rc_lib:special/3
scoreboard players set x_min rc_lib.var 12
scoreboard players set x_max rc_lib.var 36
scoreboard players set y_min rc_lib.var 20
scoreboard players set y_max rc_lib.var 44
scoreboard players set z_min rc_lib.var 8
scoreboard players set z_max rc_lib.var 16
function rc_lib:special/3
scoreboard players set x_min rc_lib.var 12
scoreboard players set x_max rc_lib.var 36
scoreboard players set y_min rc_lib.var 20
scoreboard players set y_max rc_lib.var 44
scoreboard players set z_min rc_lib.var 48
scoreboard players set z_max rc_lib.var 56
function rc_lib:special/3
scoreboard players set x_min rc_lib.var 0
scoreboard players set x_max rc_lib.var 48
scoreboard players set y_min rc_lib.var 8
scoreboard players set y_max rc_lib.var 56
scoreboard players set z_min rc_lib.var 16
scoreboard players set z_max rc_lib.var 48
function rc_lib:special/3