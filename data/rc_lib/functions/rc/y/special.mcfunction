...
execute if score liquid rc_lib.setting matches 1 if block ~ ~ ~ #rc_lib:special[waterlogged=true] run function rc_lib:rc/y/end
function rc_lib:special/branch
scoreboard players operation temp rc_lib.var = y_l rc_lib.var
